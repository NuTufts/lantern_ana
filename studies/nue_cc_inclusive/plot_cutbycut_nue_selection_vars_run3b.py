...
output_png_prefix = "run3b_cutbycut"
plot_folder = "output_cutbycut_plots_run3b"
os.system(f"mkdir -p {plot_folder}")

samples = ['nue_sig','nue_bg','numu','extbnb','data']
#samples = ['nue_sig','nue_bg','numu']

scaling = {"numu":targetpot/8.98323351831587e+20,
           "nue_sig":targetpot/4.702159572049976e+22,
           "nue_bg":targetpot/4.702159572049976e+22,
           "extbnb":23090946.0/58677653.0*2.0,
           "data":1.0}
files = {
    "numu":"./output_nue_run3/mcc9_v29e_dl_run3b_bnb_nu_overlay_20251224_040320.root",
    "nue_sig":"./output_nue_run3/mcc9_v29e_dl_run3b_bnb_intrinsic_nue_overlay_20251224_034308.root",
    "nue_bg":"./output_nue_run3/mcc9_v29e_dl_run3b_bnb_intrinsic_nue_overlay_20251224_034308.root",
    "extbnb":"./output_nue_run3/mcc9_v29e_dl_run3_G1_extbnb_dlreco_20251224_040957.root",
    "data":"./output_nue_run4b_surprise/run4b_beamon_20251224_042512.root"
}
...
vars = [
    ...
    ('vertex_properties_recoEnuMeV',50,0,5000.0,';reco neutrino energy (MeV)',0,False),
    ('visible_energy',15,0,3000,';visible energy (MeV)',0,True),
    #('trueNu_Enu',30,0,3.0,';true Neutrino Energy (GeV)',0,True)
]
...
for var, nbins, xmin, xmax, htitle, setlogy, ismcvar in vars:
    ...
    for icut,cutname in enumerate(cut_stages):
        ...
        if not ismcvar and (x_cutvar,"extbnb") in hists:
            hstack.Add( hists[(x_cutvar,"extbnb")])
        if (x_cutvar,"numu") in hists:
            hstack.Add( hists[(x_cutvar,"numu")])
        hstack.Add( hists[(x_cutvar,"nue_sig")])
        if (x_cutvar,"nue_bg") in hists and hists[(x_cutvar,"nue_bg")].Integral()>0.0:
        ...
        if not ismcvar and (x_cutvar,"data") in hists:
