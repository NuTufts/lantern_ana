...
rfile = rt.TFile("output_xsecflux_nue_cc_inclusive_run1.root")

TARGET_POT=4.4e19
MCSAMPLE_POT=9.662529168587103e+22
...
def make_hist_w_errors( rfile, varname, sample, parlist, pot_scale=1.0 ):
    ...
    for parname in parlist:
        ...
        for ibin in range(1,hvar_tot.GetXaxis().GetNbins()+1):
            ...
            hists[parname].SetBinError(ibin,sqrt(cv_var))
...
for var in variables:
    for sample in samples:
        ...
        c = rt.TCanvas(f"c{var}_{sample}",f"{var} {sample}",3000,1000)
        c.Divide(2,1)
        c.Draw()
        c.cd(1)
        c.cd(1).SetGridx(1)
