...
hnn_pefrac = rt.TH1D("hnn_pefrac",";total PE fractional error",50,-1.0,5.0)
hub_pefrac = rt.TH1D("hub_pefrac",";total PE fractional error",50,-1.0,5.0)
hnn_pefrac.SetLineColor(rt.kRed)
hnn_pefrac.SetLineWidth(2)
hub_pefrac.SetLineWidth(2)
hnn_pefrac.GetYaxis().SetLabelSize(0.05)
hub_pefrac.GetYaxis().SetLabelSize(0.05)
hnn_pefrac.GetXaxis().SetLabelSize(0.05)
hub_pefrac.GetXaxis().SetLabelSize(0.05)
hnn_pefrac.GetXaxis().SetTitleSize(0.06)
hub_pefrac.GetXaxis().SetTitleSize(0.06)
...
#analysis_tree.Draw("(flashpred_ublm_totpe-flashpred_observed_totpe)/(flashpred_observed_totpe)>>hub_pefrac",f"flashpred_observed_totpe>0 && ({numu_cut})")
hpefrac_max = 0.0
pefrac_max = 0.0
...
cfracerr.Update()
cfracerr.SaveAs(f"{output_plot_folder}/{plot_label}_pe_fracerr.png")
...
href_totpe = rt.TH1D("href_totpe",";total PE",100,0.0,15000.0)
hnn_totpe.SetLineColor(rt.kRed)
href_totpe.SetLineColor(rt.kBlack)
href_totpe.SetLineWidth(3)
hub_totpe.SetLineWidth(2)
hnn_totpe.SetLineWidth(2)
for h in [hnn_totpe,hub_totpe,href_totpe]:
    h.GetXaxis().SetLabelSize(0.05)
    h.GetYaxis().SetLabelSize(0.05)    
    h.GetXaxis().SetTitleSize(0.06)

analysis_tree.Draw("flashpred_cosmic_totpe>>href_totpe",f"flashpred_cosmic_totpe>=0.0 && ({numu_cut})")
...
for h in [hnn_totpe,hub_totpe,href_totpe]:
    if h.GetMaximum()>totpe_max:
        ...
        htotpe_max = h
#htotpe_max.GetXaxis().SetTitleOffset(1.1)
        
htotpe_max.Draw()
href_totpe.Draw("same")
hnn_totpe.Draw("same")
hub_totpe.Draw("same")
ctotpe.Update()
ctotpe.SaveAs(f"{output_plot_folder}/{plot_label}_totpe.png")
...
cpeperpmt.Update()
cpeperpmt.SaveAs(f"{output_plot_folder}/{plot_label}_pe_per_pmt.png")
...
clowpeperpmt.Update()
clowpeperpmt.SaveAs(f"{output_plot_folder}/{plot_label}_lowpe_per_pmt.png")
...
hnn_diffpmtpe.Draw()
cdiffpmtpe.Update()
cdiffpmtpe.SaveAs(f"{output_plot_folder}/{plot_label}_diff_pmtpe.png")
...
cbalsinkdiv.Update()
cbalsinkdiv.SaveAs(f"{output_plot_folder}/{plot_label}_balanced_sinkhorn_divergence.png")
...
cunbsinkdiv.Update()
cunbsinkdiv.SaveAs(f"{output_plot_folder}/{plot_label}_unbalanced_sinkhorn_divergence.png")
...
cdiff_unbsinkdiv.SaveAs(f"{output_plot_folder}/{plot_label}_diff_unbalanced_sinkhorn.png")
...
cdiff_balsinkdiv.SaveAs(f"{output_plot_folder}/{plot_label}_diff_balanced_sinkhorn.png")
