...
def load_detector_abs_unc(detsys_files, variable, cv_hist):
    """
    Compute per-bin absolute detector uncertainty in quadrature.

    For each detector parameter p and merged analysis bin i:
      sigma_i^(p) = sum_k x_k^var - sum_k x_k^cv
    where k runs over all original detsys bins inside merged analysis bin i.

    This is the absolute shift in event counts for that parameter.
    Everything stays in absolute units — no division by CV, no multiplication
    by mu_i. This avoids any ambiguity between the detsys CV and the
    xsecflux CV (which come from different MC samples).

    Then sum in quadrature over all 9 parameters:
      sigma_i^det = sqrt( sum_p (sigma_i^(p))^2 )

    Returns a numpy array of absolute uncertainties (in event counts), one per bin.
    This gets added to the diagonal as (sigma_i^det)^2.
    """
    nbins  = cv_hist.GetNbinsX()
    short  = variable.replace("numuCC1piNpReco_", "")
    abs_var = np.zeros(nbins)   # sum of squared absolute shifts
    ...
    for param in DETSYS_PARAMS:
        for rfile in detsys_files:
            ...
            for ibin in range(1, nbins + 1):
                ...
                # Absolute shift for this parameter in this bin
                abs_shift = sum_var - sum_cv
                abs_var[ibin-1] += abs_shift**2   # sum in quadrature

            break   # found this param in this file, move on

    print(f"  [{short}] detector: loaded {n_loaded}/{len(DETSYS_PARAMS)} parameters")
    return np.sqrt(abs_var)   # absolute uncertainty per bin (event counts)
...
def compute_chi2(variable, data_hist, cv_hist, V_xsecflux, det_abs_unc):
    ...
    where V_stat and V_det are diagonal only:
      ...
      V_det[i,i]  = (sigma_det[i])^2   where sigma_det is absolute shift in counts
    ...
    for i in range(nbins):
        ...
        V_total[i, i] += det_abs_unc[i]**2              # detector systematic
