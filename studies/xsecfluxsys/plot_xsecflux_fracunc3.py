...
if __name__ == "__main__":
    ...
    # ── Deep-dive: CV counts for all flagged bins ───────────────────────────
    OUTLIER_THRESHOLD = 0.15

    print(f"\n{'='*90}")
    print("DEEP DIVE: CV counts for all flagged bins (ratio deviates >15% from 1.0)")
    print(f"{'='*90}")

    for s in short_names:
        flagged = []
        for ibin_idx, ratio in enumerate(all_ratios[s]):
            ibin = ibin_idx + 1
            if abs(ratio - 1.0) > OUTLIER_THRESHOLD and ratio > 0:
                flagged.append(ibin)
        ...
        print(f"\n  {s}: flagged bins = {flagged}")
        print(f"  {'bin':>4}  {'CV count':>12}  {'ratio':>10}  {'note'}")
        print(f"  {'-'*50}")
        for ibin in range(1, all_nbins[s] + 1):
            ratio  = all_ratios[s][ibin - 1]
            cv_val = all_cv[s][ibin - 1]
            is_flagged = ibin in flagged
            marker = " <-- FLAGGED" if is_flagged else ""
            # always print flagged bins; also print neighbors for context
            if is_flagged or any(abs(ibin - f) <= 1 for f in flagged):
                print(f"  {ibin:>4}  {cv_val:>12.4f}  {ratio:>10.4f}{marker}")
