#!/usr/bin/env python3
"""
Producer Template Generator for Lantern Analysis Framework

This script creates a template file for building new producer classes.
Perfect for undergraduate students learning to create custom analysis variables!

Usage:
    python3 generate_producer_template.py MyCustomProducer
    
This will create: lantern_ana/producers/MyCustomProducer.py
"""
...
def generate_producer_template(producer_name: str, output_dir: str = None):
    """
    Generate a complete producer template with beginner-friendly documentation.
    
    Args:
        producer_name: Name of the producer class (e.g., "MyCustomProducer")
        output_dir: Directory to save the file (default: lantern_ana/producers/)
    """
    
    # Set default output directory
    if output_dir is None:
        output_dir = "lantern_ana/producers"
    
    # Create the template content
    template_content = f'''# {producer_name}.py
"""
Custom Producer: {producer_name}
...
class {producer_name}(ProducerBaseClass):
    """
    ...
    def __init__(self, name: str, config: Dict[str, Any]):
        ...
        self.min_energy = config.get('min_energy', 50.0)  # Minimum energy threshold (MeV)
        ...
        # Example variables - REPLACE THESE with your own!
        self.total_energy = array('f', [0.0])           # Total energy in event
        self.particle_count = array('i', [0])           # Number of particles
        self.energy_ratio = array('f', [0.0])           # Track/shower energy ratio
        self.max_angle = array('f', [0.0])              # Maximum opening angle
        self.is_interesting = array('i', [0])           # Boolean flag (0 or 1)
    ...
    def prepareStorage(self, output_tree):
        ...
        output_tree.Branch(f"{{self.name}}_total_energy", self.total_energy, f"{{self.name}}_total_energy/F")
        output_tree.Branch(f"{{self.name}}_particle_count", self.particle_count, f"{{self.name}}_particle_count/I")
        output_tree.Branch(f"{{self.name}}_energy_ratio", self.energy_ratio, f"{{self.name}}_energy_ratio/F")
        output_tree.Branch(f"{{self.name}}_max_angle", self.max_angle, f"{{self.name}}_max_angle/F")
        output_tree.Branch(f"{{self.name}}_is_interesting", self.is_interesting, f"{{self.name}}_is_interesting/I")
...
producers:
  my_physics_vars:                    # Your chosen name
    type: {producer_name}             # Must match your class name
...
def main():
    ...
    print("🔬 Lantern Analysis Framework - Producer Template Generator")
    print("=" * 60)
    
    # Check command line arguments
    if len(sys.argv) < 2:
        print("❌ Error: Please provide a producer name!")
        print("\nUsage:")
        print("  python3 generate_producer_template.py MyProducerName")
        print("\nExamples:")
        print("  python3 generate_producer_template.py TrackShowerRatioProducer")
        print("  python3 generate_producer_template.py EventTopologyProducer")
        print("  python3 generate_producer_template.py MissingEnergyProducer")
        sys.exit(1)
    
    producer_name = sys.argv[1]
    
    # Validate producer name
    if not producer_name.isidentifier():
        print(f"❌ Error: '{producer_name}' is not a valid Python class name!")
        print("Producer names must:")
        print("- Start with a letter or underscore")
        print("- Contain only letters, numbers, and underscores")
        print("- Not be a Python keyword")
        sys.exit(1)
    
    if not producer_name.endswith("Producer"):
        print(f"⚠️  Warning: Producer name '{producer_name}' doesn't end with 'Producer'")
    ...
    try:
        filepath = generate_producer_template(producer_name)
        print(f"✅ Success! Created producer template:")
        print(f"   📄 {filepath}")
        print()
        print("📋 Next steps for students:")
        print("1. Open the generated file in your favorite editor")
        print("2. Read through all the comments and instructions")
        print("3. Replace the example calculations with your own")
        print("4. Test with a small dataset first (max_events: 100)")
        print("5. Add your producer to the YAML configuration")
        print()
        print("📚 For help, see:")
        print("   - STUDENT_GUIDE.md for detailed tutorials")
        print("   - QUICK_REFERENCE.md for syntax help")
        print("   - studies/ directory for working examples")
        print()
