...
if __name__ == "__main__":
	...
	# POT and trigger information for Run 4b (from getDataInfo.py)
	# You'll need to update these with your actual values
	end1cnt_run4b = 10928307.0  # E1DCNT_wcut for beam on
	pot_run4b = 4.498e+19       # tor875_wcut for beam on
	...
	mc_pot_nue = 2.3593114985024402e+20  # Example - update with actual value
	...
	samples = [
		{
			'analysis_file': analysis_base + "run4b_v10_04_07_20_BNB_nu_overlay_retuple.root",
			'covar_file': covar_base + "xsecflux_run4b_nu.root",
			'dataset_name': "run4b_nu",
			'label': r'$\nu_\mu$ CC (background)',
			'color': 'blue',
			'pot_scale': pot_run4b / mc_pot_nu,
			'cut_string': nu_cuts,
			'include_uncertainties': True,  # Only stat errors (sys errors zeroed)
			'is_data': False
		},
		{
			'analysis_file': analysis_base + "run4b_v10_04_07_09_BNB_nue_overlay_surprise.root",
			'covar_file': covar_base + "xsecflux_run4b_nue.root",
			'dataset_name': "run4b_nue",
			'label': r'$\nu_e$ CC (signal)',
			'color': 'red',
			'pot_scale': pot_run4b / mc_pot_nue,
			'cut_string': nue_cuts,
			'include_uncertainties': True,  # Stat + sys errors
			'is_data': False
		},
		{
			'analysis_file': analysis_base + "run4b_dirt.root",
			'label': 'Dirt',
			'color': 'orange',
			'pot_scale': pot_run4b / mc_pot_dirt,
			'cut_string': mmr_cuts,
			'include_uncertainties': True,  # Only stat errors
			'is_data': False
		},
		{
			'analysis_file': analysis_base + "run4b_extbnb.root",  # Update with actual filename
			'label': 'EXT (beam off)',
			'color': 'gray',
			'pot_scale': end1cnt_run4b / ext_beamoff,
			'cut_string': mmr_cuts,
			'include_uncertainties': True,  # Only stat errors
			'is_data': False  # Treat as MC for stacking
		},
		{
			'analysis_file': analysis_base + "run4b_beamon.root",  # Update with actual filename
			'label': 'Data (beam on)',
			'color': 'black',
			'pot_scale': 1.0,
			'cut_string': mmr_cuts,
	...
	plot_stacked_with_systematics(
		...
		title="Run 4b Reconstructed Neutrino Energy"
	)