...
if run_num == 1: 
	...
	xsecflux_files = {
		"nue": f"{lantern_dir}/studies/xsecfluxsys/run1_outputs/xsecflux_run1_numu_intrinsic_nue.root",
		"numu": f"{lantern_dir}/studies/xsecfluxsys/run1_outputs/xsecflux_run1_numu_overlay_nu.root"
	}
	show_data = True 
	plot_title = "Run 1"
	out_name = f"{lantern_dir}/zev_mmr/hists_xsecflux/numu_hists1.root"
...
if run_num == 3: 
	...
	scaling = {"numu":targetpot/8.98323351831587e+20,
			...
			"data":1.0}
	files = {"numu": f"{lantern_dir}/run3_mmr/root_files/selection/run3b_bnb_nu_overlay_20260112_154048.root",
			"nue":f"{lantern_dir}/run3_mmr/root_files/selection/run3b_bnb_nue_overlay_20260112_155555.root", 
			"extbnb":f"{lantern_dir}/run3_mmr/root_files/selection/run3b_extbnb_20260112_160141.root",
			"data":f"{lantern_dir}/run3_mmr/root_files/selection/run3b_data_20260112_175802.root"}
	xsecflux_files = {
		"nue": f"{lantern_dir}/run3_mmr/root_files/xsecflux/run3b_numu_covar_nue.root",
		"numu": f"{lantern_dir}/run3_mmr/root_files/xsecflux/run3b_numu_covar_nu.root"
	}
	show_data = True 
	plot_title = "Run 3b"
	out_name = f"{lantern_dir}/run3_mmr/plots/plots/numu_hists3b.root"
	...
	'muon_momentum': 'reco_muon_energy',
...
for var_name, var_info in variables.items():
	...
	if h_total_mc is not None and h_uncertainty_total is not None:
		...
		# Set styles
		h_frac_stat.SetFillColor(rt.kGray+1)  # Gray for statistical
		h_frac_stat.SetFillStyle(1001)
		h_frac_stat.SetLineColor(rt.kGray+1)
		
		h_frac_flux.SetFillColor(rt.kRed)  # Red for flux
		h_frac_flux.SetFillStyle(1001)
		h_frac_flux.SetLineColor(rt.kRed)
		
		h_frac_xsec.SetFillColor(rt.kBlue)  # Blue for xsec
		h_frac_xsec.SetFillStyle(1001)
		h_frac_xsec.SetLineColor(rt.kBlue)
		
		h_frac_reint.SetFillColor(rt.kOrange)  # Orange for reint
		h_frac_reint.SetFillStyle(1001)
		h_frac_reint.SetLineColor(rt.kOrange)
		
		# Create THStack for fractional uncertainties
		hstack_frac = rt.THStack(f"hs_frac_{var_name}", "")
		hstack_frac.Add(h_frac_stat.Clone())
		hstack_frac.Add(h_frac_flux.Clone())
		hstack_frac.Add(h_frac_xsec.Clone())
		hstack_frac.Add(h_frac_reint.Clone())
		
		# Get title parts
		title_parts = var_info['title'].split(';')
		x_axis_title = title_parts[1].strip() if len(title_parts) > 1 else ""
		
		# Draw stacked fractional errors
		frac_title = f"{plot_title}; {x_axis_title}; Fractional Uncertainty"
		hstack_frac.SetTitle(frac_title)
		hstack_frac.Draw("hist")
		
		# Draw total as black line on top
		h_frac_total.SetLineColor(rt.kBlack)
		h_frac_total.SetLineWidth(2)
		h_frac_total.SetFillStyle(0)
		h_frac_total.Draw("histsame")
		
		# Set axis properties
		hstack_frac.GetXaxis().SetLabelSize(0.04)
		hstack_frac.GetXaxis().SetTitleSize(0.04)
		hstack_frac.GetYaxis().SetLabelSize(0.04)
		hstack_frac.GetYaxis().SetTitleSize(0.04)
		hstack_frac.GetYaxis().SetTitle("Fractional Uncertainty")
		...
		leg_frac.AddEntry(h_frac_stat, "Statistical", "f")
		leg_frac.AddEntry(h_frac_flux, "Flux", "f")
		leg_frac.AddEntry(h_frac_xsec, "Cross Section", "f")
		leg_frac.AddEntry(h_frac_reint, "Reinteraction", "f")
