...
# Cumulative cut branches in selection order.
# Each branch should equal 1 if the event passes all cuts up to and including that step.
# Update branch names to match what is actually in your ntuples.
# cut_flow_cuts = [
#     ("nueIncCC_cut1_vertex_fiducial",        "Fiducial\nVertex"),
#     ("nueIncCC_cut2_cosmic_rejection",        "& Cosmic Rejection"),
#     ("nueIncCC_cut3_no_muon_tracks",          "& No Muon Tracks"),
#     ("nueIncCC_cut4_has_electron",            "& Has Electron"),
#     ("nueIncCC_cut5_low_muon_score",         "& Muon Score"),
#     ("nueIncCC_cut6_electron_confidence",    "& Electron Confidence"),
#     # ("nueIncCC_passes_all_cuts",             "All\nCuts"),
# ]
...
if run_num == 1: 
	...
	# files = {"numu": f"{lantern_dir}/all_runs_mmr/run1/root_files/selection_v4/run1_nu_20260406_191414.root",
	# 		"nue":f"{lantern_dir}/all_runs_mmr/run1/root_files/selection_v4/run1_nue_20260406_191934.root", 
	# 		"extbnb":f"{lantern_dir}/all_runs_mmr/run1/root_files/selection_v4/run1_extbnb_20260406_192726.root",
	# 		"data":f"{lantern_dir}/all_runs_mmr/run1/root_files/selection_v4/run1_data_5e19_20260406_192936.root"}
...
if run_num == 3: 
	...
	files = {
			...
			"nue":f"{lantern_dir}/all_runs_mmr/run3/root_files/selection/run3b_nue_20260326_014840.root", 
	...
	plot_title = "Run 3: CC Inclusive Nue (SURPRISE NU)"
	out_name = f"{lantern_dir}/all_runs_mmr/plots/nue/nue_run3_hists.root"
	xsecflux_variables = ['visible_energy', 'reco_electron_momentum', 'reco_cos_theta']
	xsecflux_sample_map = {
		'nue': 'run3mil_nue',
		'numu': 'run3mil_nu'  
	}
...
categories = {
	...
	'nc_nue': {
		'samples': ['nue'],
		'truth_cut': f' && (nueIncCC_is_charge_current==0)',
		'color': rt.kViolet-1,
		'fill_style': 1001,
		'legend': 'NC nue'
	},
...
for var_name, var_info in variables.items():
	...
	if 'true_var' in var_info:
		...
		# truth_cut = ("(nueIncCC_true_ccnc==0)")
...
if show_cut_flow:
	...
	print(f"  Efficiency denominator (true CC nue in FV): {n_true_total:.2f}")
	...
	for cut_branch, cut_label in cut_flow_cuts:
		...
		cc_cut      = f"({cut_str}) && (nueIncCC_is_charge_current==1)"
		cc_cut_eff  = f"({cut_str}) && (nueIncCC_is_charge_current==1) && ({truth_cut_eff})"
	...
	# --- Canvas ---
	canvas_cf = rt.TCanvas("c_cutflow", "Cut Flow Efficiency and Purity", 1200, 650)
	canvas_cf.Draw()
	canvas_cf.cd()
