...
def run_plots(run_num, truth_mode=False, fully_contained=False, show_pc=True, debug=False):
	...
	show_cut_flow = True
	cut_flow_cuts = [
		("nueIncCC_cut1_vertex_fiducial",        "1"),
		("nueIncCC_cut2_cosmic_rejection",        "1,2"),
		("nueIncCC_cut3_no_muon_tracks",          "1-3"),
		("nueIncCC_cut4_has_electron",            "1-4"),
		("nueIncCC_cut5_low_muon_score",         "1-5"),
		("nueIncCC_cut6_electron_confidence",    "All"),
	]
	...
	if run_num == 3: 
		...
		scaling = {
				...
				"data":1.0}
		files = {
				"numu": f"{lantern_dir}/all_runs_mmr/run3/root_files/selection_v2/run3_nu_20260408_135518.root",
				"nue":f"{lantern_dir}/all_runs_mmr/run3/root_files/selection_v2/run3_nue_CV_20260409_140944.root", 
				"extbnb":f"{lantern_dir}/all_runs_mmr/run3/root_files/selection_v2/run3_extbnb_20260408_143338.root",
				"data":f"{lantern_dir}/all_runs_mmr/run3/root_files/selection_v2/run3_data_20260409_142513.root"}
		xsecflux_files = {
		...
		if USE_SURPRISE_NUMU:
			...
			files['numu'] = f"{lantern_dir}/all_runs_mmr/run3_surprise/root_files/selection/run3surp_nu_20260507_213849.root"
			plot_title += " (Surprise Numu)"
	...
	def merge_overflow(h):
		"""Merge the overflow bin content/error into the last visible bin, then zero it."""
		n = h.GetNbinsX()
		h.SetBinContent(n, h.GetBinContent(n) + h.GetBinContent(n + 1))
		h.SetBinError(n,   sqrt(h.GetBinError(n)**2 + h.GetBinError(n + 1)**2))
		h.SetBinContent(n + 1, 0.0)
		h.SetBinError(n + 1, 0.0)

	def add_overflow_label(h, angle=45, size=0.032):
		"""Relabel the last bin 'Overflow' without affecting other axis labels."""
		h.GetXaxis().ChangeLabel(-1, angle, size, 33, 1, 42, "Overflow")
	...
	categories = {
		...
		'nc_nue': {
			'samples': ['nue'],
			'truth_cut': f' && (nueIncCC_true_ccnc==1)',
			'color': rt.kViolet-1,
			'fill_style': 1001,
			'legend': 'NC nue'
		},
		'cc_nue': {
			'samples': ['nue'], 
			'truth_cut': f' && (nueIncCC_true_ccnc==0)',
			'color': rt.kRed-4,
			'fill_style': 1001,
			'legend': 'CC nue'
		},
	...
	for var_name, var_info in variables.items():
		...
		for cat_name, cat_info in categories.items():
			...
			for sample in cat_info['samples']:
				nue_veto = " && (remove_true_nue_cc_flag==0)" if sample == 'numu' else ""
				full_cut    = f"({base_cut}){cat_info['truth_cut']}{var_info['cut_suffix']}{nue_veto}"
		...
		# Merge overflow into last visible bin when requested
		if var_info.get('has_overflow', False):
			for cat_name in list(hists.keys()):
				merge_overflow(hists[cat_name])
			for cat_name in list(hists_unscaled.keys()):
				merge_overflow(hists_unscaled[cat_name])
		...
		# Merge overflow in xsecflux histograms to stay consistent with the merged MC hists
		if var_info.get('has_overflow', False):
			for xsn in ['nue', 'numu']:
				if xsn in xsecflux_hists and xsecflux_var in xsecflux_hists[xsn]:
					xh = xsecflux_hists[xsn][xsecflux_var]
					for key in ['cv', 'fluxvar', 'xsecvar', 'reintvar']:
						if key in xh:
							merge_overflow(xh[key])
					for h in xh.get('param_vars', {}).values():
						merge_overflow(h)
		...
		if not show_ratio:
			...
			if var_info.get('has_overflow', False):
				if stack_max > data_max:
					add_overflow_label(hstack.GetHistogram())
				elif 'data' in hists:
					add_overflow_label(hists['data'])
		canvas.Update()
		...
		if h_total_mc is not None and h_uncertainty_total is not None:
			...
			if var_info.get('has_overflow', False):
				add_overflow_label(h_frac_total)
		...
		if 'cc_nue' in hists and h_total_mc is not None:
			h_purity = hists['cc_nue'].Clone(f"h_purity_{var_name}")

			for ibin in range(0, h_purity.GetNbinsX() + 2):
				signal = hists['cc_nue'].GetBinContent(ibin)
			...
			total_signal = hists['cc_nue'].Integral()
		...
		if 'true_var' in var_info:
			...
			truth_cut = "(nueIncCC_true_ccnc==0 && true_vertex_properties_dwall>3.0)"
			...
			if run_num == 4:
				...
				for run_label in run4_labels:
					...
					trees[run_label]['nue'].Draw(f"{var_info['true_var']}>>{hname_denom}_temp",
												 f"({truth_cut})*eventweight_weight", "goff")
					h_eff_denom_temp.Scale(scaling[run_label]['nue'])
					h_eff_denom.Add(h_eff_denom_temp)
			else:
				trees['nue'].Draw(f"{var_info['true_var']}>>{hname_denom}",
								   f"({truth_cut})*eventweight_weight", "goff")
				h_eff_denom.Scale(scaling['nue'])
			...
			if run_num == 4:
				...
				for run_label in run4_labels:
					...
					trees[run_label]['nue'].Draw(f"{var_info['true_var']}>>{hname_numer}_temp",
												 f"({full_cut_eff})*eventweight_weight", "goff")
					h_eff_numer_temp.Scale(scaling[run_label]['nue'])
					h_eff_numer.Add(h_eff_numer_temp)
			else:
				trees['nue'].Draw(f"{var_info['true_var']}>>{hname_numer}",
								   f"({full_cut_eff})*eventweight_weight", "goff")
				h_eff_numer.Scale(scaling['nue'])

			if var_info.get('has_overflow', False):
				merge_overflow(h_eff_denom)
				merge_overflow(h_eff_numer)
	...
	if show_cut_flow:
		...
		if run_num == 4:
			...
			for run_label in run4_labels:
				...
				trees[run_label]['nue'].Draw(
					"0.5>>h_cf_denom_tmp",
					f"({truth_cut_eff})*eventweight_weight", "goff")
				h_cf_denom_tmp.Scale(scaling[run_label]['nue'])
				h_cf_denom.Add(h_cf_denom_tmp)
		else:
			trees['nue'].Draw(
				"0.5>>h_cf_denom",
				f"({truth_cut_eff})*eventweight_weight", "goff")
			h_cf_denom.Scale(scaling['nue'])

		n_true_total = h_cf_denom.GetBinContent(1)
		dprint(f"  Efficiency denominator (true CC nue in FV): {n_true_total:.2f}")
		...
		h_tot_pre = rt.TH1D("h_cf_tot_precut", "", 1, 0, 1)
		h_tot_pre.Sumw2()
		if run_num == 4:
			h_tot_pre_tmp = rt.TH1D("h_cf_tot_precut_tmp", "", 1, 0, 1); h_tot_pre_tmp.Sumw2()
			for run_label in run4_labels:
				for sample in ['nue', 'numu', 'extbnb']:
					h_tot_pre_tmp.Reset()
					veto_pre = nue_veto if sample == 'numu' else ""
					trees[run_label][sample].Draw("0.5>>h_cf_tot_precut_tmp", f"(1{veto_pre})*eventweight_weight", "goff")
					h_tot_pre_tmp.Scale(scaling[run_label][sample])
					h_tot_pre.Add(h_tot_pre_tmp)
		else:
			for sample in ['nue', 'numu', 'extbnb']:
				h_tmp_pre = rt.TH1D(f"h_cf_tot_precut_{sample}", "", 1, 0, 1); h_tmp_pre.Sumw2()
				veto_pre = nue_veto if sample == 'numu' else ""
				trees[sample].Draw(f"0.5>>h_cf_tot_precut_{sample}", f"(1{veto_pre})*eventweight_weight", "goff")
		...
		if run_num == 4:
			...
			h_fc_tmp      = rt.TH1D("h_nue_cf_fc_tmp",  "", 1, 0, 1); h_fc_tmp.Sumw2()
		...
		for cut_branch, cut_label in cut_flow_cuts:
			...
			cc_cut      = f"({cut_str}) && (nueIncCC_true_ccnc==0)"
			cc_cut_eff  = f"({cut_str}) && (nueIncCC_true_ccnc==0) && ({truth_cut_eff})"
			...
			if run_num == 4:
				for run_label in run4_labels:
					...
					h_sig_tmp.Scale(scaling[run_label]['nue'])
					h_sig_eff_tmp.Scale(scaling[run_label]['nue'])
					h_sig.Add(h_sig_tmp)
				...
				trees['nue'].Draw(
					...
					f"({cc_cut_eff})*eventweight_weight", "goff")
				h_sig.Scale(scaling['nue'])
				h_sig_eff.Scale(scaling['nue'])
			...
			if fully_contained or show_pc:
				h_fc = rt.TH1D(f"h_nue_cf_fc_{cut_branch}", "", 1, 0, 1)
				h_fc.Sumw2()
				fc_str = f"{cut_str} && (nueIncCC_is_fully_contained==1)"
				if run_num == 4:
					for run_label in run4_labels:
						for sample in ['nue', 'numu', 'extbnb']:
							h_fc_tmp.Reset()
							veto = nue_veto if sample == 'numu' else ""
							trees[run_label][sample].Draw(f"0.5>>h_nue_cf_fc_tmp", f"({fc_str}{veto})*eventweight_weight", "goff")
							h_fc_tmp.Scale(scaling[run_label][sample])
							h_fc.Add(h_fc_tmp)
				else:
					for sample in ['nue', 'numu', 'extbnb']:
						h_fc_s = rt.TH1D(f"h_nue_cf_fc_{cut_branch}_{sample}", "", 1, 0, 1)
						h_fc_s.Sumw2()
						veto = nue_veto if sample == 'numu' else ""
						trees[sample].Draw(f"0.5>>h_nue_cf_fc_{cut_branch}_{sample}", f"({fc_str}{veto})*eventweight_weight", "goff")
		...
		h_removed_cf = rt.TH1D("h_cutflow_removed", "", n_cuts, 0, n_cuts)
		h_removed_cf.SetFillColor(rt.kOrange + 7)
		h_removed_cf.SetLineColor(rt.kOrange + 9)

		stack_rem = rt.THStack("stack_cutflow_removed", "")

		if show_pc:
			h_kept_fc_cf = rt.TH1D("h_cutflow_kept_fc", "", n_cuts, 0, n_cuts)
			h_kept_pc_cf = rt.TH1D("h_cutflow_kept_pc", "", n_cuts, 0, n_cuts)
			...
			h_kept_cf = rt.TH1D("h_cutflow_kept", "", n_cuts, 0, n_cuts)
			for i in range(n_cuts):
		...
		canvas_rem = rt.TCanvas(f"c_cutflow_removed_{run_num}", "Events Cut and Kept at Each Cut", 1200, 650)
		canvas_rem.Draw()
		canvas_rem.cd()
		...
		stack_rem.SetTitle(f"Nue Run {run_num}: Number of Events by Cut")
