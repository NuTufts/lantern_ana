...
if run_num == 4: 
	...
	show_data = False 
	plot_title = "Run 4b"
	# plot_title = "Run4b: Inclusive CC #nu_{#mu} Selection"
...
# Define sample categories with requested colors - for muon neutrino analysis
categories = {
	'cosmic': {
		'samples': ['extbnb'],
...
# Define variables to plot - updated for muon neutrino analysis
variables = {
	'neutrino_energy': {
...
for var_name, var_info in variables.items():
	...
	for cat_name, cat_info in categories.items():
		...
		for sample in cat_info['samples']:
			...
			# For neutrino energy: manually handle overflow by putting values >= 2.0 into last bin
			...
			# For muon momentum: manually handle overflow by adjusting values > 1.5 to be exactly 1.5
			elif var_name == 'muon_momentum':
				# Create a temporary histogram to capture all events including overflow
				temp_hist = rt.TH1D(hname+"_temp", "", 1000, 0, 10)  # Large range to capture all events
				trees[sample].Draw(f"{var_info['var']}>>{hname}_temp", f"({full_cut})*eventweight_weight", "goff")
			...
			# For cos theta: no special handling needed
			elif var_name == 'muon_costheta':
				# Standard filling
			...
			# For data, scale the errors appropriately (sqrt(N) * scale_factor)
			if sample == 'data':
				for ibin in range(1, hist.GetNbinsX() + 1):
					content = hist.GetBinContent(ibin)
					# For Poisson statistics after scaling: error = sqrt(content)
					error = sqrt(content) if content > 0 else 0
	...
	if rebin_config:
		...
		if rebin_type == 'variable':
			...
			# Constant rebinning factor (for muon_momentum)
			...
			# Range restriction (for muon_costheta: 0-1)
	...
	if total_mc > 0:
		...
		print(f"νe CC background: {cc_nue_events/total_mc*100:.1f}%") 
