# To run: python3 plot_tki.py <run_number> [contained|uncontained]
# e.g.    python3 plot_tki.py 1
#         python3 plot_tki.py 3 uncontained
#         python3 plot_tki.py 3 contained   (default)
...
if run_num == 1:
    ...
    plot_title   = f"Run 1: TKI ({sel_label.capitalize()})"
    out_name     = f"{lantern_dir}/organized_all_run_yamls/plots_run1/tki_run1_{sel_label}_withsys_{timestamp}.root"
    ...
    if contained:
        xsecflux_file = f"{lantern_dir}/studies/xsecfluxsys/systematics_output_run1_march2026_contained/output_xsecflux_tki_run1_march2026_contained.root"
        detsys_files  = [
        ...
        xsecflux_file = f"{lantern_dir}/studies/xsecfluxsys/systematics_output_run1_march2026_uncontained/output_xsecflux_tki_run1_march2026_uncontained.root"
        detsys_files  = [
...
if run_num == 3:
    ...
    sel_label    = "contained" if contained else "uncontained"
    plot_title   = f"Run 3: TKI ({sel_label.capitalize()})"
    out_name     = f"{lantern_dir}/organized_all_run_yamls/plots_run3/tki_run3_{sel_label}_withsys_{timestamp}.root"
    ...
    if contained:
        xsecflux_file = f"{lantern_dir}/studies/xsecfluxsys/output_tki_run3b_1mil_xsecflux_contained/output_xsecflux_tki_run3b_1mil_contained.root"
        detsys_files  = [
            f"{lantern_dir}/studies/xsecfluxsys/detsys_run3b_1mil_mcc9_v29e_dl_run3b_bnb_nu_overlay_1mil_contained.root",
            f"{lantern_dir}/studies/xsecfluxsys/detsys_cv500k_run3b_bnb_nu_overlay_500k_CV_contained.root",
        ]
    else:
        xsecflux_file = f"{lantern_dir}/studies/xsecfluxsys/output_tki_run3b_1mil_xsecflux_uncontained/output_xsecflux_tki_run3b_1mil_uncontained.root"
        detsys_files  = [
            f"{lantern_dir}/studies/xsecfluxsys/detsys_run3b_1mil_mcc9_v29e_dl_run3b_bnb_nu_overlay_1mil_uncontained.root",
            f"{lantern_dir}/studies/xsecfluxsys/detsys_cv500k_run3b_bnb_nu_overlay_500k_CV_uncontained.root",
        ]
...
def load_detector_variations(rfiles, varname, detsys_params, nbins, xmin, xmax):
    ...
    for param in detsys_params:
        for rfile in rfiles:
            ...
            for ibin in range(0, h_frac_var.GetNbinsX() + 2):
                bin_centre = h_frac_var.GetXaxis().GetBinCenter(ibin)
                det_ibin   = h_cv.GetXaxis().FindBin(bin_centre)
                cv_val  = h_cv.GetBinContent(det_ibin)
                var_val = h_var.GetBinContent(det_ibin)
                if cv_val > 0:
...
if detsys_files:
    ...
    if dfiles:
        for var, nbins, xmin, xmax, _ in vars:
            short = var.replace("numuCC1piNpReco_", "")
            result = load_detector_variations(dfiles, short, detsys_params, nbins, xmin, xmax)
            if result:
...
for i, (var, nbins, xmin, xmax, htitle) in enumerate(vars):  # FIX: enumerate for unique hnames
    ...
    hname_denom = f"h_{short}_{i}_effdenom"  # FIX: index i in name
    ...
    # ── Fill each sample ──────────────────────────────────────────────────────
    for sample in samples:
        hname          = f"h_{short}_{i}_{sample}"           # FIX: index i in name
        hname_unscaled = f"h_{short}_{i}_{sample}_unscaled"  # FIX: index i in name
    ...
    hname_num = f"h_{short}_{i}_effnum"  # FIX: index i in name
    ...
    # ── Total MC ──────────────────────────────────────────────────────────────
    h_total_mc = hists["numu_sig"].Clone(f"h_{short}_{i}_total_mc")           # FIX
    h_total_mc.Add(hists["numu_bg"])
    h_total_mc_unscaled = hists_unscaled["numu_sig"].Clone(f"h_{short}_{i}_total_mc_unscaled")  # FIX
    h_total_mc_unscaled.Add(hists_unscaled["numu_bg"])

    # ── Build uncertainty band ────────────────────────────────────────────────
    h_unc_total    = h_total_mc.Clone(f"h_{short}_{i}_unc_total");    h_unc_total.Reset()    # FIX
    h_unc_flux     = h_total_mc.Clone(f"h_{short}_{i}_unc_flux");     h_unc_flux.Reset()     # FIX
    h_unc_xsec     = h_total_mc.Clone(f"h_{short}_{i}_unc_xsec");     h_unc_xsec.Reset()     # FIX
    h_unc_reint    = h_total_mc.Clone(f"h_{short}_{i}_unc_reint");    h_unc_reint.Reset()    # FIX
    h_unc_detector = h_total_mc.Clone(f"h_{short}_{i}_unc_detector"); h_unc_detector.Reset() # FIX
    ...
    for ibin in range(0, h_total_mc.GetNbinsX() + 2):
        ...
        if short in xsecflux_hists:
            ...
            if short == 'muKE':
                print(f"  ibin={ibin} bin_centre={bin_centre:.1f} xs_ibin={xs_ibin} fluxvar={xs['fluxvar'].GetBinContent(xs_ibin):.6f}")
    ...
    hstack = rt.THStack(f"hs_{short}_{i}", "")  # FIX: unique THStack name
    ...
    h_purity = hists["numu_sig"].Clone(f"h_{short}_{i}_purity")          # FIX
    h_sum    = hists["numu_sig"].Clone(f"h_{short}_{i}_sum")              # FIX
    h_sum.Add(hists["numu_bg"])
    h_sum.Add(hists["extbnb"])

    h_sum_unscaled = hists_unscaled["numu_sig"].Clone(f"h_{short}_{i}_sum_unscaled")  # FIX
    ...
    h_eff = hists["effnum"].Clone(f"h_{short}_{i}_eff")  # FIX
    ...
    if xsecflux_hists or detsys_hists:
        ...
        h_frac_stat     = h_total_mc.Clone(f"h_{short}_{i}_frac_stat");     h_frac_stat.Reset()     # FIX
        h_frac_flux     = h_total_mc.Clone(f"h_{short}_{i}_frac_flux");     h_frac_flux.Reset()     # FIX
        h_frac_xsec     = h_total_mc.Clone(f"h_{short}_{i}_frac_xsec");     h_frac_xsec.Reset()     # FIX
        h_frac_reint    = h_total_mc.Clone(f"h_{short}_{i}_frac_reint");    h_frac_reint.Reset()    # FIX
        h_frac_detector = h_total_mc.Clone(f"h_{short}_{i}_frac_detector"); h_frac_detector.Reset() # FIX
        h_frac_total    = h_total_mc.Clone(f"h_{short}_{i}_frac_total");    h_frac_total.Reset()    # FIX
