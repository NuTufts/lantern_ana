...
if run_num == 1:
    ...
    # Systematics — separate files for contained vs uncontained
    ...
    if contained:
        ...
        detsys_files  = [
            f"{lantern_dir}/studies/xsecfluxsys/detsys_run3b_1mil_mcc9_v29e_dl_run3b_bnb_nu_overlay_1mil_contained.root",  # 7 params (using Run 3 detsys, contained)
            f"{lantern_dir}/studies/xsecfluxsys/detsys_cv500k_run3b_bnb_nu_overlay_500k_CV_contained.root",  # SCE + recomb2
        ]
    else:
        ...
        detsys_files  = [
            f"{lantern_dir}/studies/xsecfluxsys/detsys_run3b_1mil_mcc9_v29e_dl_run3b_bnb_nu_overlay_1mil_uncontained.root",  # 7 params (uncontained)
            f"{lantern_dir}/studies/xsecfluxsys/detsys_cv500k_run3b_bnb_nu_overlay_500k_CV_uncontained.root",                 # SCE + recomb2 (uncontained)
        ]
...
if run_num == 3:
    ...
    if contained:
        ...
        detsys_files  = [
            f"{lantern_dir}/studies/xsecfluxsys/detsys_run3b_1mil_mcc9_v29e_dl_run3b_bnb_nu_overlay_1mil_contained.root",  # 7 params (contained)
            f"{lantern_dir}/studies/xsecfluxsys/detsys_cv500k_run3b_bnb_nu_overlay_500k_CV_contained.root",  # SCE + recomb2 (contained)
        ]
    else:
...
def make_hist_w_errors(rfile, varname, sample, parlist):
    ...
    for parname in parlist:
        ...
        for ibin in range(0, hvar_tot.GetNbinsX() + 2):
            ...
            # Store fractional variance: (sigma/CV)^2
            # Scale variance from mean to CV, then normalise by CV^2
            if xmean > 0 and cv_val > 0:
                frac_var = xvar / xmean**2   # = (sigma/mean)^2, i.e. already fractional
            else:
                frac_var = 0.0
...
def load_detector_variations(rfiles, varname, detsys_params, nbins, xmin, xmax):
    ...
    combined fractional-variance histogram on the ANALYSIS binning.
    Naming: hnumuCC1piNpReco_{var}__{param}__cv  /  __var
    """
    # Create h_frac_var on the analysis binning (not the detsys binning)
    ...
    for param in detsys_params:
        for rfile in rfiles:
            ...
            for ibin in range(0, h_frac_var.GetNbinsX() + 2):
                # Map analysis bin centre onto detsys histogram (may have coarser binning)
...
for var, nbins, xmin, xmax, htitle in vars:
    ...
    for ibin in range(0, h_total_mc.GetNbinsX() + 2):
        ...
        # fluxvar/xsecvar/reintvar now store fractional variance (sigma/mean)^2 directly.
        # Use FindBin to map analysis bin centre onto the xsecflux histogram bin.
        frac_var_flux  = 0.0
        frac_var_xsec  = 0.0
        frac_var_reint = 0.0
